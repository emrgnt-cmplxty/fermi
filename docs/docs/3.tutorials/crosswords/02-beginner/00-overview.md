...
Let's give the smart contract the ability to store multiple crosswords and offer the winner a prize, paid in Fermi.
...
- Allow users to log in with a Fermi account
- Give a prize (in Fermi tokens) to the first person to solve the puzzle
...
As we implement the list above, we'll learn key concepts about Fermi:
...
- Fermi's specialized [Collections](https://docs.near.org/concepts/storage/data-storage#rust-collection-types) that are generally preferable to, say, Rust's standard HashMap 
