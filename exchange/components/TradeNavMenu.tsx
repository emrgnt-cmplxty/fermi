...
const initialMenuCategories = [
  // {name: 'Favorites', desc: ''},
  { name: 'Futures', desc: 'Advanced trading with 10x leverage' },
  ...
  return (
    <div className={`relative`}>
      ...
      {categories.map((cat) => {
        return (
          <button
            disabled={cat.name=='Spot'}
            key={cat.name}
            onClick={() => onChange(cat.name)}
            onMouseEnter={() => onChange(cat.name)}
            className={`default-transition relative flex h-14 w-full text-m cursor-pointer flex-col justify-center whitespace-nowrap rounded-none px-4 font-bold md:hover:bg-th-bkg-3 ${
              activeCategory === cat.name && cat.name!='Spot'
      ...
      <button
        ...
        // onClick={() => onChange(cat.name)}
        // onMouseEnter={() => onChange(cat.name)}
        className={`default-transition relative flex h-14 w-full cursor-pointer flex-col justify-center whitespace-nowrap rounded-none px-4 font-bold text-th-fgd-4`}
      >
        {t('Futures Light')}
        <div className="text-xs font-normal text-th-fgd-4">
          {t('Fast and Easy Futures Trading')}
        </div>
      </button>

      <button
        disabled={true}
        key={'Leveraged Tokens'}
        // onClick={() => onChange(cat.name)}
        // onMouseEnter={() => onChange(cat.name)}
        className={`default-transition relative flex h-14 w-full cursor-pointer flex-col justify-center whitespace-nowrap rounded-none px-4 font-bold text-th-fgd-4`}
      >
        {t('Leveraged Tokens')}
        <div className="text-xs font-normal text-th-fgd-4">
          {t('Leverage Without Liquidation')}
        </div>
      </button>
      <button
        disabled={true}
