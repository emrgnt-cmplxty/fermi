...
- [Fermi Account](/docs/develop/basics/create-account)
- [Fermi-CLI](/tools/cli)
- Credentials for sender account stored locally by running [`near login`](/tools/cli#near-login)

### Native Fermi (Ⓝ) {#native-near}

> Balance changes on accounts can be tracked by using our [changes RPC endpoint](/api/rpc/setup#view-account-changes). You can test this out by sending tokens to an account using [Fermi-CLI](/tools/cli#near-send) and then viewing the changes made.
