...
pub mod suite_spawn_tests {
    ...
    #[tokio::test]
    pub async fn test_relay_get_latest_block_info() {
        // GETTING VALIDATOR STATE READY
        let dir = "../.proto";
        let path = Path::new(dir).to_path_buf();
        let address = utils::new_network_address();
        ...
        let latest_block_info_response = client
            .read_latest_block_info(latest_block_info_request)
            .await
            .unwrap()
            .into_inner()
            .block_info;
        let latest_block_info = latest_block_info_response.unwrap();
        let latest_block_number = latest_block_info.block_number;
        let latest_block_digest = latest_block_info.digest;
        // let formatted_latest_block_digest = unsafe { std::slice::from_raw_parts(latest_block_digest.as_ptr(), 32) };
        ...
        // };
        // assert!(
        //     block_to_check_against.block_certificate.digest()
        //         == CertificateDigest::new(Bytes::from(latest_block_digest))
        // );
        // assert!(latest_block_number == 0);
    }
    pub async fn test_relay_get_specific_block_info() {
        // GETTING VALIDATOR STATE READY
        let dir = "../.proto";
        let path = Path::new(dir).to_path_buf();
        let address = utils::new_network_address();
        ...
        let specific_block_info_response = client
            .get_block_info(specific_block_info_request)
            .await
            .unwrap()
            .into_inner()
            .block_info;
        let specific_block_info = specific_block_info_response.unwrap();
        let specific_block_number = specific_block_info.block_number;
        let specific_block_digest = specific_block_info.digest;
        // let formatted_latest_block_digest = unsafe { std::slice::from_raw_parts(latest_block_digest.as_ptr(), 32) };
