...
use gdex_controller::{
    ...
    spot::controller::{
        SpotController,
        SPOT_CONTROLLER_ACCOUNT_PUBKEY,
...
use gdex_types::{
    ...
    crypto::ToFromBytes, order_book::OrderSide
};
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    bank_controller.lock().unwrap().create_account(&controller_account).unwrap();
    ...
    for i in 0..n_orderbooks {
        ...
        for _ in 0..2 {
            bank_controller.lock().unwrap().create_asset(creator_kp.public()).unwrap();
        }

        // create orderbook
        let base_asset_id = 2*i;
        let quote_asset_id = 2*i+1;
        spot_controller.lock().unwrap().create_orderbook(base_asset_id, quote_asset_id).unwrap();

        for k in 1..n_users {
            let user = keypairs[k].public();

            // transfer assets from creator to user
            bank_controller.lock().unwrap().transfer(creator_kp.public(), user, base_asset_id, 10000000).unwrap();
            bank_controller.lock().unwrap().transfer(creator_kp.public(), user, quote_asset_id, 1000000).unwrap();

            // place orders to orderbook
            let bid_request = create_limit_order_request(base_asset_id, quote_asset_id, OrderSide::Bid as u64, n_users as u64 % 100 + 1, 1); //n_users as u64 % 100
            let ask_request = create_limit_order_request(base_asset_id, quote_asset_id, OrderSide::Ask as u64, n_users as u64 % 100 + 101, 1);
            spot_controller.lock().unwrap().get_orderbook(base_asset_id, quote_asset_id).unwrap().place_limit_order(user, &bid_request).unwrap();
            spot_controller.lock().unwrap().get_orderbook(base_asset_id, quote_asset_id).unwrap().place_limit_order(keypairs[k].public(), &ask_request).unwrap();
        }
    }
    ...
    c.bench_function("spot_create_catchup_state", |b| {
        b.iter(|| {
            SpotController::create_catchup_state(spot_controller.clone(), 0)
        })
    });
