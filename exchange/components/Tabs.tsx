      ...
      <nav className="-mb-px flex" aria-label="Tabs">
        {tabs.map((tabName) => {
          const tabCount = showCount
            ...
            : null
          const isDisabled = !(tabName!='vaults' && tabName!='leveraged-tokens' && tabName!='favorites')

          return (
            isDisabled?
            <div
            key={tabName}
            onClick={() => { !isDisabled ? onChange(tabName) : null}}
            className={` relative flex justify-center whitespace-nowrap pb-4 font-bold 
                  ${
                    activeTab === tabName && !isDisabled
                      ? `default-transition cursor-pointer text-th-primary hover:opacity-100`
