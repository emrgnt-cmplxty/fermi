...
use types::account::{AccountError, AccountPrivKey, AccountPubKey, AccountSignature};
...
impl ConsensusManager {
    ...
    pub fn propose_block(&self, txns: Vec<TxnRequest<TxnVariant>>) -> Result<Block<TxnVariant>, AccountError> {
        ...
        Ok(Block::<TxnVariant>::new(
            txns,
            self.validator_pub_key,
            block_hash,
            self.hash_clock.get_time(),
            vote_cert,
        ))
    }
...
#[cfg(test)]
mod tests {
    use super::super::router::{order_transaction, orderbook_creation_txn, payment_txn};
    use super::*;
    ...
    fn test_simple_consensus() {
        ...
        let mut txns: Vec<TxnRequest<TxnVariant>> = Vec::new();
        let signed_txn: TxnRequest<TxnVariant> =
            asset_creation_txn(secondary_pub_key, secondary_validator.get_validator_private_key()).unwrap();
        route_transaction(&mut primary_validator, &signed_txn).unwrap();
        txns.push(signed_txn);
