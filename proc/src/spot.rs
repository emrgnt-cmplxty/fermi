...
impl <'a> SpotController <'a>
{
    pub fn new(base_asset_id: AssetId, quote_asset_id: AssetId, bank_controller: &'a mut BankController) -> Self {
    ...
    pub fn place_limit_order(&mut self, account_pub_key: &AccountPubKey, side: OrderSide, qty: u64, price: u64) -> Result<OrderProcessingResult, AccountError> {
        ...
        if matches!(side, OrderSide::Ask) { 
            ...
            assert!(self.bank_controller.get_balance(account_pub_key, self.base_asset_id)  > qty);
        } else { 
        ...
        for order in &res {
            match order {
                ...
                Ok(Success::PartiallyFilled{order_id, side, price, qty, ..}) => {
                    ...
                    self.proc_order_fill(&existing_pub_key, *side, *price, *qty);
                },
    ...
    fn proc_order_init(&mut self, account_pub_key: &AccountPubKey, side: OrderSide, price: u64, qty: u64) {
        if matches!(side, OrderSide::Ask) { 
            ...
            self.bank_controller.update_balance(account_pub_key, self.quote_asset_id, -((qty * price) as i64));
        }
    }
    
    // process a filled order by modifying the associated account
    fn proc_order_fill(&mut self, account_pub_key: &AccountPubKey, side: OrderSide, price: u64, qty: u64) {
        if matches!(side, OrderSide::Ask) { 
    ...
    pub fn get_bank_controller(&mut self) -> &mut BankController {
        return &mut self.bank_controller;
    }
