...
> When you deploy a smart contract to Axion, you pay for the storage that this contract requires using a mechanism called storage staking.
>
...
3. When such a call comes in, Axion will check that `example.near` has a large enough balance that it can stake an amount to cover the new storage needs. If it does not, the transaction will fail.
...
Note that a call to your smart contract to remove data has an associated gas fee. Given Axion's gas limit, this creates an upper limit on how much data can be deleted in a single transaction.

## How much does it cost? {#how-much-does-it-cost}

Storage staking is priced in an amount set by the network, which is set to **1E19 yoctoNEAR per byte**, or **100kb per Axion token (Ⓝ)**. [^1] [^2]

Axion's JSON JSON-RPC provides [a way to query this initial setting](/api/rpc/setup#genesis-config) as well as a [a way to query the live config / recent blocks](/api/rpc/setup#protocol-config).
...
3. Let's assume well-formed Axion `AccountId`s, and let's guess that Axion Account IDs follow the approximate pattern of domain names, which [average about 10 characters](https://www.gaebler.com/Domain-Length-Research.htm), plus a top-level name like `.near`. So a reasonable average to expect might be about 15 characters; let's keep our estimate pessimistic and say 25. This will equal 25 bytes, since Axion account IDs must use characters from the ASCII set.
...
Storing data on-chain isn't cheap for the people running the network, and Axion passes on this cost to developers. So, how do you, as a developer, keep your costs down? There are two popular approaches:
...
The core Axion team maintains a library called [borsh](https://borsh.io/),
...
Let's use this [Guest Book](https://github.com/near-examples/guest-book) as an example. As implemented today, visitors to the app can sign in with Axion and leave a message. Their message is stored on-chain.
...
Axion's structure incentivizes network operators while giving flexibility and predictability to contract developers. Managing storage is an important aspect of smart contract design, and Axion's libraries make it easy to figure out how much storage will cost for your application.
