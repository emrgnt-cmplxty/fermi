// crate
use super::types::{CondensedOrder, FuturesMarket, Marketplace, OpenOrder, Position};
// gdex
...
pub(crate) fn combine_positions(mut old_position: Position, new_position: Position) -> Option<Position> {
...
pub(crate) fn condense_orders(open_orders: &[OpenOrder], base_asset_id: u64) -> (CondensedOrder, CondensedOrder) {
...
pub(crate) fn compute_realized_pnl(
    ...
    resultant_position: &Option<Position>,
    price: u64,
    ...
    let multiplier = if old_position.side == OrderSide::Bid { 1 } else { -1 };
...
pub(crate) fn account_market_req_collateral(
    ...
    if let Some(position) = position {
        ...
        if position.side == OrderSide::Bid {
    ...
    for market in market_place.markets.values() {
        if let Some(account) = market.accounts.get(account) {
            ...
            if order_data.base_asset_id == market.base_asset_id && order_data.quantity + condensed_bids.quantity > 0 {
                if order_data.side == OrderSide::Bid {
