...
class GDEXBench:
    ...
    def setup_genesis(self, bench_parameters_dict, benchmark=True, release=True):
        ...
        Print.info(f"Running {cmd}")
        subprocess.run(cmd, check=True, cwd=PathMaker.fermi_build_path())
        sleep(0.5)  # Removing the store may take time.
        ...
        cmd = CommandMaker.init_fermi_genesis(os.path.abspath(bench_parameters.key_dir))
        Print.info(f"Running {cmd}...")
        subprocess.run([cmd], shell=True)
        ...
        for filename in key_files:
            cmd = CommandMaker.generate_fermi_key(filename, os.path.abspath(bench_parameters.key_dir)).split()
            Print.info(f"Running {cmd}...")
        ...
        cmd = CommandMaker.add_controllers_fermi_genesis(os.path.abspath(bench_parameters.key_dir))
        Print.info(f"Running {cmd}")
        subprocess.run([cmd], shell=True)
        cmd = CommandMaker.build_fermi_genesis(os.path.abspath(bench_parameters.key_dir))
        Print.info(f"Running {cmd}")
        subprocess.run([cmd], shell=True)

        for i, name in enumerate(committee.json['authorities'].keys()):
            cmd = CommandMaker.verify_and_sign_fermi_genesis(os.path.abspath(bench_parameters.key_dir), os.path.abspath(bench_parameters.key_dir + key_files[i]))
            Print.info(f"Running {cmd}")
            subprocess.run([cmd], shell=True)
        Print.info(f"Running {cmd}")
