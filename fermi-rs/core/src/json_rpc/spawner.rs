...
mod test_json_rpc_server {
    use fermi_controller::futures::test::futures_tests::FuturesControllerTester;
    use fermi_controller::futures::types::{MarketResponse, MarketplaceResponse, MarketplaceUserInfoResponse};
    use fermi_controller::router::ControllerRouter;
    use fermi_controller::ControllerTestBed;
    use fermi_types::crypto::KeypairTraits;
    use fermi_types::order_book::{OrderSide, OrderbookDepth};
    use fermi_types::{
    ...
    async fn test_get_markets() -> anyhow::Result<()> {
        ...
        assert!(expected_market.max_leverage == fermi_controller::futures::test::futures_tests::TEST_MAX_LEVERAGE);
        assert!(expected_market.base_asset_id == fermi_controller::futures::test::futures_tests::BASE_ASSET_ID);
        assert!(expected_market.quote_asset_id == fermi_controller::futures::test::futures_tests::QUOTE_ASSET_ID);
        assert!(expected_market.open_interest == 0);
        assert!(expected_market.last_traded_price == 0);
        assert!(expected_market.oracle_price == fermi_controller::futures::test::futures_tests::INITIAL_ASSET_PRICES[0]);
        Ok(())
    }
        ...
        assert!(
            response.user_collateral_req
                == (maker_quantity - taker_quantity) * maker_price
                    / fermi_controller::futures::test::futures_tests::TEST_MAX_LEVERAGE
                    + taker_quantity * fermi_controller::futures::test::futures_tests::INITIAL_ASSET_PRICES[0]
                        / fermi_controller::futures::test::futures_tests::TEST_MAX_LEVERAGE
                    + 1
        );
        assert!(
            response.user_unrealized_pnl
                == (fermi_controller::futures::test::futures_tests::INITIAL_ASSET_PRICES[0] - maker_price) as i64
    ...
    async fn test_get_orderbook_snapshot_info() -> anyhow::Result<()> {
        ...
        futures_tester
            .generate_orderbook_depths(
                rpc_store,
                fermi_controller::futures::controller::ORDERBOOK_DEPTH_FREQUENCY,
            )
            .await;
