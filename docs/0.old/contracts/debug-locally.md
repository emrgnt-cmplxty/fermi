...
### Reproduce a `testnet` Cross Contract Call on a Sandbox Node {#reproduce-a-testnet-cross-contract-call-on-a-sandbox-node}
...
#### Fetch Information and Reproduce Locally {#fetch-information-and-reproduce-locally}

Now let's reproduce this cross-contract call locally.

1. Spin up a sandbox node.
...
You should see that the status has been successfully set to "world". If you're debugging a contract bug and want to avoid regression in the future, it's recommended to keep the genesis generated by `get-sandbox0repro-genesis.js` as a fixture and write a test script to run the transaction on a sandbox node.

## Obtain Information from Testnet/Mainnet Manually {#obtain-information-from-testnetmainnet-manually}
