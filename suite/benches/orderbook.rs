...
pub fn criterion_benchmark(c: &mut Criterion) {
    ...
    group.bench_function("place_orders_engine", |b| {
        b.iter(|| place_orders_engine(base_asset_id, quote_asset_id, black_box(N_ORDERS_BENCH), &mut rng, &db, false))
    });

    group.bench_function("place_orders_engine_account", |b| {
        b.iter(|| place_orders_engine_account(black_box(N_ORDERS_BENCH), base_asset_id, quote_asset_id, &mut account_to_pub_key, &mut bank_controller, &mut orderbook_controller, &mut rng, &db, false))
    });

    // w/ write-out to db
    group.bench_function("place_orders_engine_db", |b| {
        b.iter(|| place_orders_engine(base_asset_id, quote_asset_id, black_box(N_ORDERS_BENCH), &mut rng, &db, true))
    });

    group.bench_function("place_orders_engine_account_db", |b| {
        b.iter(|| place_orders_engine_account(black_box(N_ORDERS_BENCH), base_asset_id, quote_asset_id, &mut account_to_pub_key, &mut bank_controller, &mut orderbook_controller, &mut rng, &db, true))
    });
}
