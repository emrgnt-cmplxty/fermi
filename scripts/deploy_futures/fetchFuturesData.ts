...
import { TenexClient } from 'tenex-sdk'
...
async function main() {
    ...
    const takerPublicKey = await FermiAccount.getPublicKey(FermiUtils.hexToBytes(takerAuthority.private_key))
    const marketTakerData = await client.getUserMarketplaceInfo(marketPlace.admin, FermiUtils.bytesToHex(takerPublicKey))
