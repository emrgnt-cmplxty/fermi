...
The [Axion Platform overview](/docs/concepts/overview) clarifies much of the language in this section.
...
- The RPC interface receives the transaction and routes it to the correct physical node using `signer_id`.  Since the `signer_id` must be a Axion Account ID which lives on a single shard, the account is mapped to a shard which is followed by at least one validator running at least one machine with an IP address.
...
## Axion Platform Errors {#near-platform-errors}
...
- **VMerror and subtypes**: errors raised during the execution of a Wasm contract by the Axion VM
...
```text
VMerror                                                   An error that occurs in the Axion virtual machine
