...
const FloatingElement: FunctionComponent<FloatingElementProps> = ({
  ...
  return (
    <div
      className={`thin-scroll relative overflow-auto overflow-x-hidden  bg-th-bkg-2 p-2.5 md:p-4 ${className}`}
    >
      {!connected && showConnect && !pubkey ? (
        <div className="absolute top-0 left-0 z-10 h-full w-full">
          ...
          <div className="absolute top-0 left-0 h-full w-full  bg-th-bkg-2 opacity-50" />
        </div>
      ) : null}
      {!uiLocked ? (
        <div className="absolute top-0 left-0 z-50 h-full w-full cursor-move opacity-80">
          ...
          <div className="absolute top-0 left-0 h-full w-full  bg-th-bkg-3" />
        </div>
      ) : null}
      {children}
    </div>
