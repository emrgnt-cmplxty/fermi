...
import { PerpMarketInfo } from '@gdexorg/ifm-client'
import configFile from './tempConfig.json';
...
export const ENDPOINTS: EndpointInfo[] = [
  ...
  {
    name: 'localnet',
    url: 'https://rpc.gdex.fi',
    websocket: 'wss://rpc.gdex.fi ',
    custom: false,
  },
]

type ClusterType = 'mainnet' | 'devnet' | 'testnet' | 'localnet'
// const DEFAULT_MANGO_GROUP_NAME = process.env.NEXT_PUBLIC_GROUP || 'mainnet.1'
const DEFAULT_MANGO_GROUP_NAME = 'mango_test_v2'
// hardcode cluser to localnet
...
//   (t) => t.symbol === 'MNGO'
//)
const config_ = new Config(configFile);
const groupConfig = config_.getGroupWithName(DEFAULT_MANGO_GROUP_NAME) as GroupConfig;

export const MNGO_INDEX = 0;

export const programId = new PublicKey(groupConfig!.mangoProgramId)

export const serumProgramId = new PublicKey(
  groupConfig!.serumProgramId
)
const mangoGroupPk = new PublicKey(groupConfig!.publicKey)
    ...
    return {
      ...
      actions: {
        ...
        async fetchMangoGroup() {
          ...
          const connection = get().connection.current
          const actions = get().actions
          const mangoGroup = await mangoClient.getMangoGroup(mangoGroupPk)
          const mangoClientData = await mangoClient.connection.getAccountInfo(mangoGroupPk);
        ...
        async fetchMarketsInfo() {
          ...
          try {
            ...
            if (data?.status === 200) {
              ...
              const filteredMarketsInfo = [];
              const activeMarketsInfo = configFile["groups"][0]["perpMarkets"];

              for (const parsedMarket of parsedMarketsInfo) {
                 for (const activeMarket of activeMarketsInfo) {
                    if (activeMarket["name"] == parsedMarket["name"]) {
                      filteredMarketsInfo.push(parsedMarket);
                    }
                 }
              }
              const remainingMarkets = ["ALGO", "MATIC", "DOGE", "XRP", "AAPL", "META", "GOLD", "SILVER"];
              for (const market of remainingMarkets) {
                filteredMarketsInfo.push({
                  "name": market+"-PERP",
                  "baseSymbol": market,
                  "baseVolume24h": 0.0,
                  "quoteVolume24h": 0.0,
              ...
              console.log("filteredMarketsInfo", filteredMarketsInfo);
              set((state) => {
                state.marketsInfo = filteredMarketsInfo
              })
            }
          } catch (e) {
