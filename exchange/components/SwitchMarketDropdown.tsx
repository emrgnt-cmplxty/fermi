...
const descTextBySymbol = {
  'AVAX': 'Avalanche Futures',
  'BTC': 'Bitcoin Futures',
  'ETH': 'Ethereum Futures',
  'SOL': 'Solana Futures',
  'BNB': 'Binance Coin Futures',
  'XRP': 'Ripple Futures',
  'DOGE': 'Dogecoin Futures',
  'MATIC': 'MATIC Futures',
  'ALGO': 'ALGO Futures',
  'TSLA': 'Tesla Futures',
  'META': 'Meta Futures',
    ...
    <Popover>
      {({ open }) => (
        <div className="relative flex flex-col">
          <Popover.Button
            className={`p-0.5 transition-none  focus:outline-none ${
              open && 'border-th-fgd-4'
            }`}
            ref={buttonRef}
          >
            <div className="flex items-center pl-2 -mt-4 mb-4">
              <div >
                <img
                  alt=""
                  width="24"
                  ...
                  className={`mr-2.5 mt-3`}
                />
              </div>
              <div className="pr-0.5  ">{baseSymbol}</div>
              <span className="text-xll text-th-fgd-4">
                {isPerpMarket ? '' : '/'}
              </span>
              <div className="pl-0.5 text-xl ">
                {isPerpMarket ? '' : groupConfig?.quoteSymbol}
              </div>

              <div
                className={`flex h-10 w-8 -ml-2 -mr-1 items-center justify-center rounded-none`}
              >
                {/* <ChevronDownIcon
              ...
              {/* <div className="relative  -mt-1 mb-1 ml-11 text-xxs text-th-fgd-3">{'Perpetual'}</div> */}
            </div>
            <Grid className="-mt-7 mb-7 text-xxs text-th-fgd-3" style={{height:0}} sx={{ml:5.25}}>{descTextBySymbol[baseSymbol]}</Grid>
          ...
          >
            <Popover.Panel
              className="thin-scroll absolute left-10 top-3 z-10 max-h-[50vh] w-72 transform overflow-y-auto rounded-b-md rounded-tl-md bg-th-bkg-3 p-4 sm:max-h-[75vh]"
              tabIndex={-1}
            >
