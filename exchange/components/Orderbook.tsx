...
export default function Orderbook({ depth = 8 }) {
  ...
  useInterval(() => {
    // if (
    //   true || nextOrderbookData?.current?.bids &&
...
//     <hr
//         className={"mb-2 -mt-1 bg-th-bkg-2"}
//         style={{
//           'height': '0px',
              ...
              <div className="relative flex -mb-7">
                  ...
                  >
                    <button
                      ...
                      className="flex h-7 w-7 items-center justify-center rounded-full bg-th-bkg-4 focus:outline-none md:hover:text-th-primary  -ml-1"
                    >
          ...
          <FlipCardBack>
            <FloatingElement className="fadein-floating-element h-full">
              ...
              <div className={`mb-2 flex justify-between text-th-fgd-4`}>
                ...
                <div className={`text-right text-xs`}>
                  {t('total')} ({marketConfig.baseSymbol})
                </div>
              </div>
              {orderbookData?.asks.map(
                ({
              ...
              <div className="my-2 flex justify-between rounded-md bg-th-bkg-1 p-2 text-xs">
                ...
                <div className={`flex items-center justify-center  md:w-1/3 md:text-base text-th-primary `}>
                    <>
                    <Tooltip
                      content={t('oracle-price-use')}
                      className="py-1 text-xs "
                    >
                    <FlagIcon/> 
                    </Tooltip>
                  </> 
                  { commaFormatter( (oraclePrice?.toNumber() || 0)/ 1000)  }
                </div>
              </div>
              {orderbookData?.bids.map(
                ({
...
const OrderbookRow = React.memo<any>(
  ({
    ...
    const handleCumSizeClick = () => {
      setMangoStore((state) => {
        state.tradeForm.baseSize = usdFormatter(Number(cumulativeSize), minOrderSizeDecimals, false)
      })
    }
          ...
          <>
            <div className="flex w-full items-center justify-between hover:font-semibold">
              <div
                ...
                onClick={both ? handlePriceClick : handleSizeClick}
              >
                {both ? usdFormatter(formattedPrice, groupingDecimalCount, false) : usdFormatter(formattedSize, minOrderSizeDecimals, false)}
              </div>
              <div
                ...
                onClick={both ? handleSizeClick : handlePriceClick}
              >
                {both ? usdFormatter(formattedSize, minOrderSizeDecimals, false) : usdFormatter(formattedPrice, groupingDecimalCount, false)}
              </div>
              {both && <div
                className={`z-10 text-xs leading-5 md:pr-4 md:leading-6 `}
                onClick={handleCumSizeClick}
              >
                {usdFormatter(cumulativeSize, minOrderSizeDecimals, false)}
              </div> }
            </div>
  ...
  ({ markPrice }) => {
    ...
    return (
      ...
      >
        ...
        {markPrice ? commaFormatter(markPrice) : ''  }
      </div>
    )
  },
