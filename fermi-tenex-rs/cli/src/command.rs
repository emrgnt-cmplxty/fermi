...
use fermi_node::faucet_server::FAUCET_PORT;
use fermi_types::{
    ...
    pub async fn execute(self) -> Result<(), anyhow::Error> {
        match self {
            GDEXCommand::Start {
                ...
                let network_config_path = config.clone().unwrap_or(fermi_config_dir()?.join(GDEX_NETWORK_CONFIG));
            ...
            GDEXCommand::Genesis {
                ...
                if write_config.is_none()
                    && fermi_config_dir
                        .read_dir()
                        .map_err(|err| {
                            anyhow!(err).context(format!("Cannot open GDEX config dir {:?}", fermi_config_dir))
                        })?
                        .next()
                        .is_some()
                {
                    if force {
                        fs::remove_dir_all(fermi_config_dir).map_err(|err| {
                            anyhow!(err).context(format!("Cannot remove GDEX config dir {:?}", fermi_config_dir))
                        })?;
                        fs::create_dir(fermi_config_dir).map_err(|err| {
                            anyhow!(err).context(format!("Cannot create GDEX config dir {:?}", fermi_config_dir))
                        })?;
                    } else {
                        bail!("Cannot run genesis with non-empty GDEX config directory {}, please use --force/-f option to remove existing configuration", fermi_config_dir.to_str().unwrap());
                    }
                }

                let network_path = fermi_config_dir.join(GDEX_NETWORK_CONFIG);
                let genesis_path = fermi_config_dir.join(GDEX_GENESIS_FILENAME);
                let gateway_path = fermi_config_dir.join(GDEX_GATEWAY_CONFIG);
                let gateway_db_folder_path = fermi_config_dir.join("gateway_client_db");
                ...
                let mut network_config = if let Some(validators) = validator_info {
                    NetworkConfigBuilder::new(fermi_config_dir)
                ...
                for validator in &mut network_config.validator_configs {
                    validator.genesis = fermi_core::config::Genesis::new_from_file(&genesis_path);
                }
                ...
                fullnode_config.save(fermi_config_dir.join(GDEX_FULLNODE_CONFIG))?;

                for (i, validator) in network_config.into_validator_configs().into_iter().enumerate() {
                    let path = fermi_config_dir.join(format!("validator-config-{}.yaml", i));
                    validator.save(path)?;
                }
            ...
            GDEXCommand::GenerateKeystore {
                ...
                let keystore_path = keystore_path.unwrap_or(fermi_config_dir()?);
